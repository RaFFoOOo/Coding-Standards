...
- **Agent Behavior Mandate:**
  ...
  - **Response Economy [MANDATORY — token discipline]:** Output is the most expensive token class; spend it deliberately. **Lead with the answer or the result**, not preamble — never restate the prompt, narrate what you are about to do, or recap what was just shown. Prefer **schematic form over prose**: tables, bullets, diffs, and short code blocks carry decisions more cheaply than paragraphs. Reserve long-form discursive explanation for genuine design/architecture/trade-off discussion that the Tech Lead must weigh; for implementation, status, and mechanical work, a 2–4 line summary plus the artifact is the target. **Do not print a full artifact in chat and also write it to a file** — generate it once at its destination and summarize (see also §1 *Artifact Generation*). This rule trims per-turn output cost; it never licenses dropping a required gate, a correctness caveat, or an honest "this failed / I skipped X" report.
