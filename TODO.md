...
- [ ] **Forward-sync this session's standards into le-cementine (deferred — timing-gated):** after `sprint/23-config-foundation` closes **and** no le-cementine session is active, run a **reviewed** `/sync-templates` pull (Coding-Standards `.agents/` → le-cementine `.claude/`). **Prioritize** the updated `sync-templates.md` (Step 5f agnostic-review gate + Step 6c shim reconciliation) and the process-rule lean (manage-artifacts dedup, recursive-review dead-ref removal). **Skip in the Step 4 diff review** the genericized *examples* (`lc-webapp`→`src`, `IOrderService`→`IFooService`, ServiceMode service names) — those would clobber le-cementine's legitimate project-specific paths/names. Do NOT initiate while a concurrent le-cementine session is live (see [[cross-repo-branch-safety]]).
