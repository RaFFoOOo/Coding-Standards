...
- [ ] **Sync-engine enhancement — `settingsStandard` (sync standard `settings.json` keys)** — make `.claude/settings.json` a *partial* sync concept so uniform project settings (e.g. `env.CLAUDE_AFK_TIMEOUT_MS`) live in one hub source instead of being hand-applied per repo (as they were 2026-07-03 across all 3 repos). NOT whole-file sync: settings.json mixes standard-worthy keys with project/personal keys (`model`, `permissions`, MCP, plugins) that must not be clobbered — so the hub declares a key allowlist (`settingsStandard`) and the engine deep-merges only those, hub-authoritative + preserve-flavor, same model as rules. Full design: [`docs/settings-standard-sync-spec.md`](docs/settings-standard-sync-spec.md). Effort: small–medium, one sprint task, no new dependency.
