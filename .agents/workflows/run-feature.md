...
4. **Determine work scope [MANDATORY]:** Inspect the task size before branching:
...
5. Read relevant skills: `.agents/skills/plan-sprint/SKILL.md`, `.agents/skills/run-qa/SKILL.md` (if exists).
...
6. Read rules: the relevant `.agents/rules/stack-*.md` file for the current tech stack and `AGENTS.md` (user global rules).
...
13. Run the appropriate build command for the stack (e.g., `npx ng build --configuration development`).
14. Check build output for errors and warnings. Fix any issues.
15. **Browser Test [OPTIONAL]:** Ask the user if they want to execute structured browser tests. If confirmed, use the `test-browser` skill (requires a browser automation tool configured in the project). If unavailable, perform manual testing and document results. Fix any failures. If skipped, proceed to the next step.
...
27. **GitHub Actions:** Audit every `.github/workflows/*.yml` file. For each action (e.g., `actions/checkout`, `actions/setup-node`):
...
29. **Report & Plan:** If ANY outdated dependencies or actions are found:
    ...
    - Automatically append upgrade tasks to the **next sprint's** `PLAN.md`.
